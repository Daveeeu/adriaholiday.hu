...
class UpdateTourSeasonalGroupRequest extends FormRequest
{
    ...
    {
        $id = $this->route('tourSeasonalGroup')?->id ?? $this->route('tourSeasonalGroup');

        return [
            'active' => ['boolean'],
            'menu_type' => ['required', 'string', 'max:255'],
            'name' => ['required', 'string', 'max:255'],
            'seo_name' => ['nullable', 'string', 'max:255', Rule::unique('tour_seasonal_groups', 'seo_name')->ignore($id)],
